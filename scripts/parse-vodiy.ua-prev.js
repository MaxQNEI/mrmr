    ...
    constructor() {
        ...
        this.Mongo
            ...
            .then(({ res, url }) => {
                ...
                domPunctList.forEach((anchor, index) => {
                    ...
                    this.PunctList[Punct] = { uuid: guid(), Punct, Title, Url: anchor.href };
            ...
            .then(() => {
                ...
                console.debug(LogStamp(), `\x1b[1;95mParsing Navigation\x1b[0m ...`);

                const pageListByPunct = {};
                const pageList = [];

                const fetchList = punctListEntries.map(([punctKey, punctItem]) => {
                    return this.fetch(punctItem.Url, {})
                        .then(({ res, url }) => {
                            const { document } = this.html2dom(res, url);

                            pageListByPunct[punctKey] = (pageListByPunct[punctKey] || []);
                            pageList.push({ url, punctItem });
                            document.querySelectorAll('.navigation_post ul li a').forEach(anchor => {
                                ...
                                const page = { url: anchor.href, punctItem };
                                pageListByPunct[punctKey].push(page);
                                pageList.push(page);
                            });
                            console.debug(LogStamp(), `\x1b[1;95mPunct Pages\x1b[0m: ${pageListByPunct[punctKey].length}`);
                ...
                return Promise.all(fetchList)
                    .then(() => {
                        return Promise.all(pageList.map(({ url, punctItem }) => {
                            return this.fetch(url, {})
                                .then(({ res, url }) => {
                                    return { res, url, punctItem };
                                });
                        }));
                    });
            })
            .then((result) => {
                console.debug(LogStamp(), `\x1b[1;95mParsing Pages\x1b[0m ...`);
                result.forEach(({ res, url, punctItem }) => {
                    const { document } = this.html2dom(res, url);

                    if (!punctItem) {
                        console.warn('punctItem:', punctItem, '->', url);
                        return;
                    }
                    ...
                    domQuestionList.forEach((item) => {
                        const ticketNumber = item.querySelector('input[name="ticket_number"]').value;
                        // const question_id = item.querySelector('input[name="question_id"]').value;
                        ...
                        this.TicketList[ticketNumber] = (this.TicketList[ticketNumber] || {
                            uuid: guid(),
                            Number: ticketNumber,
                            QuestionNumberList: [],
                        });

                        const alreadyExists = !!this.TicketList[ticketNumber].QuestionNumberList.includes(questionNumber);

                        this.TicketList[ticketNumber].QuestionNumberList.push(questionNumber);
                        ...
                        punctItem.TicketNumberList = (punctItem.TicketNumberList || []);
                        punctItem.TicketNumberList.push(ticketNumber);

                        punctItem.QuestionNumberList = (punctItem.QuestionNumberList || []);
                        punctItem.QuestionNumberList.push(questionNumber);
                        ...
                        if (domTicketLeftImg && !domTicketLeft) {
                            ...
                            this.QuestionList[questionNumber].Picture = domTicketLeftImg.src;
                        ...
                        domAnswerList.forEach(label => {
                            ...
                            if (!this.QuestionList[questionNumber].AnswerList.some(answer => answer.Text === Text)) {
                                this.QuestionList[questionNumber].AnswerList.push({ uuid: guid(), Text, isCorrect });
                            } else {
                        ...
                        this.QuestionList[questionNumber].AnswerList.length > 5 &&
                            console.debug(LogStamp(), `    \x1b[1;93mGreat than 5 answers!!\x1b[0m`);
                    });
                });
            })
            /* .then(() => {
                console.debug(LogStamp(), `\x1b[1;95mDownloading pictures\x1b[0m ...`);

                const fetchList = [];
                for (let key in this.QuestionList) {
                    if (!this.QuestionList[key].Picture) { continue; }
                    if(fetchList.length >= 3) { break; }
                    fetchList.push(
                        this
                            .fetch(this.QuestionList[key].Picture, {})
                            .then(({ res }) => {
                                return { key, base64: Buffer.from(res).toString('base64') };
                            })
                    );
                }

                return Promise
                    .all(fetchList.slice(0, 3))
                    .then(result => {
